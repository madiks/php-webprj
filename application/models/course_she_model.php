 ...
 * 
 */
class Course_she_model extends CI_Model
{
    ...
    {
        $this->db->insert('course_she', $data);
    ...
    function update($where = NULL,$data = NULL){
        $this->db->where($where);
        $this->db->update('course_she',$data);
    ...
    function read($where = NULL,$limit = NULL,$offset = NULL,$fields = NULL)
    {
        ...
        if(!empty($where)) $this->db->where($where);
        $items =  $this->db->get('course_she',$limit,$offset);
        return $items->result();
    }
    ...
    function get_count($where = NULL){
        if(!empty($where)) $this->db->where($where);
        return $this->db->count_all_results('course_she');
    }
     ...
     function getByPage($where = NULL, $pageNo = NULL, $pageCount, $order = NULL, $fields){
        ...
        $res =  $this->db->get('course_she', $pageCount, $offset);
        $data['cshes'] = $res->result();
