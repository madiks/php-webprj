          ...
          <ul class="nav navbar-nav side-nav">
            ...
            <li class="dropdown">
              ...
              <ul class="dropdown-menu">
                ...
                <li><a href="<?php echo base_url(); ?>admin/member/deal">优惠活动管理</a></li>
            ...
            <div class="col-md-11">
              <div class="panel panel-primary">
                <div class="panel-heading">
                  <h3 class="panel-title">会员申请列表(共<?php echo $tatolCount;?>条)</h3>
                  <div class="pull-right">
                    <span class="clickable filter" data-toggle="tooltip" title="检索会员申请" data-container="body">
                     <i class="fa fa-filter"></i>检索
                    </span>
                  </div>
                </div>
                <div class="panel-body">
                  <input type="text" class="form-control" id="dev-table-filter" data-action="filter" data-filters="#dev-table" placeholder="只能快速检索当页会员申请" />
                </div>
                <table class="table table-hover" id="dev-table">
                  ...
                  <?php foreach($users as $ur):?>
                    <tr id="uid<?php echo $ur->id;?>">
                      ...
                      <td>
                        <button class="btn btn-primary btn-xs btnedit" actname="<?php echo $ur->id;?>" data-title="Edit" data-toggle="modal" data-target="#edit" data-placement="top" rel="tooltip" title="处理申请"><i class="fa fa-cogs"></i></button>&nbsp
                        <button class="btn btn-danger btn-xs btndel" actname="<?php echo $ur->id;?>" data-title="Delete" data-toggle="modal" data-target="#delete" data-placement="top" rel="tooltip" title="忽略申请"><i class="fa fa-ban"></i> </button>
                      </td>
                    </tr>
                  <?php endforeach ?>
                  </tbody>
              ...
              <ul class="pager">
               <?php if($link['pageNo']>1){ ?>
                 <li class="previous"><a href="<?php echo base_url(); ?>admin/member/deal/<?php echo $link['pageNo']-1;?>">&larr; 上一页</a></li>
               <?php } ?>
               <?php if($link['pageNo']<$link['totalPageNum']){ ?>
                 <li class="next"><a href="<?php echo base_url(); ?>admin/member/deal/<?php echo $link['pageNo']+1;?>">下一页 &rarr;</a></li>
               <?php } ?>
               </ul>
            </div>
        </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title custom_align" id="Heading">处理会员申请</h4>
          </div>
          <div class="modal-body">
          <div class="form-group">
           <input class="form-control " id="uname" placeholder="真实姓名" type="text" >
           </div>
           <div class="form-group">
           
           <input class="form-control " id="cardnum" placeholder="会员卡号" type="text" >
           </div>
          ...
          <div class="modal-footer ">
            <button type="button" id="suremodify" actname="" class="btn btn-warning btn-lg" style="width: 100%;"><i class="fa fa-check"></i></span>变更为正式会员</button>
          </div>
        </div>
        <!-- /.modal-content --> 
      </div>
        ...
        <div class="modal-content">
           ...
           <div class="alert alert-warning"><span class="glyphicon glyphicon-warning-sign"></span> 确定彻底删除该会员申请（此操作不可逆）?</div>
    ...
    <!-- Page Specific Plugins -->
     <script type="text/javascript">
         ...
         $("body").on("click",".btnedit",function(){
          ...
          $("#sname").val($("#uid"+act+" td").eq(2).html());
         ...
         $("body").on("click","#suredel",function(){
           act = $(this).attr("actname");
            url = "<?php echo base_url(); ?>admin/member/del_usera/"+act;
           //alert(url);
           if(url!=""){
               window.location = url;
           }
          ...
          $("body").on("click","#suremodify",function(){
           ...
           if(Cardnum != ""){
                $.ajax({
                       ...
                       dataType:'json',    
                       url: "<?php echo base_url(); ?>admin/member/deal_userapply",
                       success: function(data){
                         ...
                         url = "<?php echo base_url(); ?>admin/member/deal";
                        window.location = url;
                     }
               }); 
             } 
